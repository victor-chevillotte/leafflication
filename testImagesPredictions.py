...
def testDir(dir_path, modelName):
    trueClasses = []
    predictedClasses = []
...
def main():
    ...
    trueClasses, predictedClasses = testDir(folder_path, modelName)
    y = 0
    for i in range(len(trueClasses)):
        if trueClasses[i] == predictedClasses[i]:
            y += 1
            print(
                f"{GREEN}True class: {trueClasses[i]}, Predicted class: {predictedClasses[i]}{RESET}"
            )
        else:
            print(
                f"{RED}True class: {trueClasses[i]}, Predicted class: {predictedClasses[i]}{RESET}"
            )
