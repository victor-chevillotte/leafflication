            ...
            if result.stdout:
                ...
                for line in lines:
                    if "Predicted class :" in line:
                        ...
                        filename_without_extension = re.sub(r'\.jpe?g$', '', file_name, flags=re.IGNORECASE)
...
def main():
    ...
    modelName = f"{args.m}.keras"
    if args.d:
        folder_path = args.d
    else:
        folder_path = 'data/test_images/Unit_test1'
    trueClasses, predictedClasses = testDir(folder_path)
    y = 0
    for i in range(len(trueClasses)):
        if trueClasses[i] == predictedClasses[i]:
            y += 1
            print(f"{GREEN}True class: {trueClasses[i]}, Predicted class: {predictedClasses[i]}{RESET}")
        else:
            print(f"{RED}True class: {trueClasses[i]}, Predicted class: {predictedClasses[i]}{RESET}")
