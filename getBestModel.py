            ...
            if filename.endswith(".json"):
                ...
                try:
                    with open(filepath, 'r') as file:
                        ...
                        if validation_accuracy is not None and validation_accuracy > 0.9:
                            note = data.get('note')
                            if note is not None:
                                note = int(note.split('/')[0])
                            else:
                                note = -1
                            accuracies.append((filename, validation_accuracy, note))
                except Exception as e:
                    print(f"An error has occured : {e}, in {filepath}")
        accuracies.sort(key=lambda x: (x[2], x[1]), reverse=True)
        for model in accuracies:
            print(f"Model : {model[0]} - Accuracy : {model[1]}, Note : {model[2]}")
